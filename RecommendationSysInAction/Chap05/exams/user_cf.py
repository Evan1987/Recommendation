...
class UserCFRecommend(object):
    ...
    def cal_user_similarity(self) -> Dict[str, Dict[str, float]]:
        ...
        user_rated_summary = list(self.user_rated_summary.items())
        n_users = len(user_rated_summary)
        for i in tqdm(range(n_users)):
            u, u_sets = user_rated_summary[i]
            for j in range(i + 1, n_users):
                v, v_sets = user_rated_summary[j]
    ...
    def evaluation(self, k: int = 8, n_items: int = 10) -> Tuple[float, float]:
        ...
        test_num = 0
        pred_num = 0
        for test_user, test_items in test_user_rated_items.items():
